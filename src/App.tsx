...
import hoverTrash from './assets/trash-hover.svg'
...
export function App() {
  ...
  function handleNewTask() {
    ...
    const taskAlreadyExists = tasks.find((task) => task.text === text)

    if (taskAlreadyExists) return

    const newTask = { id, isFinished: false, text }
  ...
  function finishTask(taskId: number, isFinished: boolean) {
    const newTasks = tasks.reduce(
      (acc, task) =>
        acc.concat(
          taskId === task.id
            ? Object.assign(task, { isFinished: !isFinished })
            : task
        ),
      [] as ITask[]
    )
    ...
    <>
      ...
      <main>
        <div className="info">
          ...
          <div className="container">
            <strong className="purple">Concluídas</strong>
            <span className="badge">{`${finisheTasks} de ${tasks.length}`}</span>
          </div>
        </div>
            ...
            {tasks.map((task) => (
              <div className="card">
                <input
                  ...
                  onChange={() => finishTask(task.id, task.isFinished)}
                />

                <span className={task.isFinished ? 'finished' : ''}>
                  {task.text}
                </span>
